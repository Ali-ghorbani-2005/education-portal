...
export default function SearchResults() {
    ...
    return (
        ...
        // </div> 
        <div className="p-6">
    <h2 className="text-3xl font-semibold text-right text-gray-700 mr-10 "> جستجو : {query}</h2>
    {results.length > 0 ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 mt-4">
            {results.map((product) => (
                <div key={product.id} className="bg-white shadow-md w-72 rounded-lg overflow-hidden mt-10 transition-transform transform hover:scale-105">
                    <div className="flex justify-center items-center p-2">
                        {product.img && <img src={product.img} alt={product.product} className="w-80 h-32 object-cover rounded-lg" />}
                    </div>
